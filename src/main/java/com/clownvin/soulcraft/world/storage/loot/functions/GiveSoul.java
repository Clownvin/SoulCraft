...
public class GiveSoul extends LootFunction {
    ...
    public GiveSoul(LootCondition[] conditionsIn) {
        super(conditionsIn);
    }
    ...
    public ItemStack apply(ItemStack stack, Random rand, LootContext context) {
        if (stack.getItem() == Items.BOOK) {
            ...
            ItemEnchantedBook.addEnchantment(stack, new EnchantmentData(SoulCraft.SOUL_ENCHANTMENT, 1));
        } else {
            stack.addEnchantment(SoulCraft.SOUL_ENCHANTMENT, 1);
        }
    ...
    public static class Serializer extends LootFunction.Serializer<GiveSoul> {
        public Serializer() {
            super(new ResourceLocation(SoulCraft.MOD_ID, "give_soul"), GiveSoul.class);
        }

        public void serialize(JsonObject object, GiveSoul functionClazz, JsonSerializationContext serializationContext) {

        }

        public GiveSoul deserialize(JsonObject object, JsonDeserializationContext deserializationContext, LootCondition[] conditionsIn) {
            return new GiveSoul(conditionsIn);
        }
    }
}
